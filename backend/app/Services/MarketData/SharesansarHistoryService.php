...
class SharesansarHistoryService
{
    ...
    public function fetchFullHistory(Stock $stock): array
    {
        ...
        try {
            ...
            Log::warning('NEPSE full-history fetch failed', ['symbol' => $stock->symbol, 'error' => $e->getMessage()]);
    ...
    /**
     * Lightweight sibling to fetchFullHistory() — just the one company-page
     * GET, no price-history pagination. Used for bulk sector backfills where
     * pulling years of prices for every stock would be wasteful.
     */
    public function fetchSector(Stock $stock): ?string
    {
        ...
        $sector = $this->extractSector($response->body());
    ...
    {
        if (! preg_match('/id="sector"[^>]*>([^<]*)</', $html, $m)) {
            return null;
        }

        $sector = trim(html_entity_decode($m[1], ENT_QUOTES | ENT_HTML5));
